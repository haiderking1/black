/**
 * OpenAI Codex, via a ChatGPT Plus or Pro subscription.
 *
 * Catalog is the set of models that backend currently serves. Chat goes through
 * the Responses endpoint on chatgpt.com, authenticated with an OAuth access
 * token and the ChatGPT account id embedded in it.
 */

import type { FetchLike, Provider } from '../types'
import { verifiedThinkingRequest } from '../thinking/validate'
import { contextWindowFor, imagesFor, thinkingFor, toolsFor } from './catalog'
import { createCodexCatalog, type CodexCatalog } from './remoteCatalog'
import { createChatClient } from './client'
import { CODEX_BASE_URL } from './endpoints'
import { PROVIDER_ID } from './oauth/constants'
import { createStreamingClient } from './stream'

export interface CodexProviderOptions {
  baseUrl?: string
  apiKey: string
  fetchImpl?: FetchLike
  catalogTtlMs?: number
  now?: () => number
}

export interface CodexProvider extends Provider {
  catalog: CodexCatalog
}

export function createCodexProvider(options: CodexProviderOptions): CodexProvider {
  const baseUrl = options.baseUrl ?? CODEX_BASE_URL
  const shared = {
    baseUrl,
    apiKey: options.apiKey,
    ...(options.fetchImpl !== undefined ? { fetchImpl: options.fetchImpl } : {}),
  }
  const client = createChatClient(shared)
  const streaming = createStreamingClient(shared)
  const catalog = createCodexCatalog({
    baseUrl,
    accessToken: options.apiKey,
    ...(options.fetchImpl !== undefined ? { fetchImpl: options.fetchImpl } : {}),
    ...(options.catalogTtlMs !== undefined ? { ttlMs: options.catalogTtlMs } : {}),
    ...(options.now !== undefined ? { now: options.now } : {}),
  })
  const model = async (modelId: string) => catalog.get(modelId)
  const thinking = async (modelId: string) => (await model(modelId))?.thinking ?? thinkingFor(modelId)

  return {
    id: PROVIDER_ID,
    name: 'OpenAI Codex',
    catalog,
    listModels: () => catalog.list(),
    refreshModels: () => catalog.refresh(),
    contextWindowFor: async (modelId) => (await model(modelId))?.context ?? contextWindowFor(modelId),
    thinkingFor: thinking,
    supportsImages: async (modelId) => (await model(modelId))?.images ?? imagesFor(modelId),
    supportsToolCalls: async (modelId) => (await model(modelId))?.tools ?? toolsFor(modelId),
    chat: async (request) => client.chat(await verifiedThinkingRequest(request, thinking)),
    streamChat: async function* (request) {
      yield* streaming.stream(await verifiedThinkingRequest(request, thinking))
    },
  }
}
