 ...
 *
 * Frames are the same SSE shape as OpenCode. Reasoning is requested with the
 * `reasoning` object, hosts with `provider`, and mid-stream failures carry
    ...
    async *stream(request: ChatRequest): AsyncGenerator<ChatStreamEvent> {
      ...
      try {
        response = await doFetch(joinUrl(options.baseUrl, CHAT_COMPLETIONS_PATH), {
          ...
          body: JSON.stringify({ ...buildChatBody(request), stream: true, stream_options: { include_usage: true } }),
