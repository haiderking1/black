...
test('transcript and composer share one column token and one gutter', async () => {
  const [column, composer, queued, floating, app, transcript] = await Promise.all([
    ...
    Bun.file('frontend/chat/virtual/VirtualTranscript.tsx').text(),
  ])

  expect(column).toMatch(/--chat-column:\s*800px/)
  expect(column).toMatch(/--chat-gutter:\s*16px/)
  expect(app).toContain('<VirtualTranscript')
  expect(transcript).toContain('chat-column chat-transcript chat-transcript-virtual')
  expect(transcript).toContain('useVirtualizer')
