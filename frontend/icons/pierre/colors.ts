import type { IconThemeMode } from './types'

export const ICON_COLORS: Record<string, readonly [light: string, dark: string]> = {
  astro: ['#a631be', '#d568ea'],
  babel: ['#d5a910', '#ffd452'],
  bash: ['#199f43', '#5ecc71'],
  biome: ['#1a85d4', '#69b1ff'],
...
export function getIconColor(token: string | undefined, theme: IconThemeMode): string {
  const colors = ICON_COLORS[token ?? 'default'] ?? ICON_COLORS.default!
  return colors[theme === 'light' ? 0 : 1]
}
